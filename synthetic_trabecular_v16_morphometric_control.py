...
def extract_single_params(voi, args):
    ...
    tbth = (
        args.tbth_um if args.tbth_um is not None
        else float(voi.get("TbTh_um_p90", voi.get("TbTh_um_p50", 0))) * c * 2.0
    )
    tbsp = (
        args.tbsp_um if args.tbsp_um is not None
        else float(voi.get("TbSp_um_p50", 0)) * c * 2.0
    )
    tbn = (
        args.tbn_per_mm if args.tbn_per_mm is not None
        else float(bv) / (float(tbth) / 1000.0)
    )
...
def tbth_um_to_radius_vox(t, v):
    return max(0.5, (t / v) / 2.0)


def tbn_per_mm_to_base_sigma(t, v):
    return float(max(1.5, 1000.0 / max(0.1, float(t)) / float(v) / 4.0))


def compute_adaptive_fill_sigma(r):
    return float(np.clip(0.35 * r, 0.3, 1.5))
...
def morph_iters(v, op, it):
    ...
    x = v.astype(bool)
    if op == "close":
        x = ndi.binary_closing(x, structure=st, iterations=int(it))
    elif op == "open":
        x = ndi.binary_opening(x, structure=st, iterations=int(it))
    return x.astype(np.uint8)


def hysteresis_on_response(R, ql, qh):
    ql = float(np.clip(ql, 0.5, 0.995))
    qh = float(np.clip(qh, ql + 1e-3, 0.999))
    th = float(np.quantile(R, qh))
    tl = float(np.quantile(R, ql))
    s = R >= th
    w = R >= tl
    st = ndi.generate_binary_structure(3, 2)
    l, n = ndi.label(w, structure=st)
    if n == 0:
        return s.astype(np.uint8), {"thr_lo": tl, "thr_hi": th}
    sl = np.unique(l[s])
    k = np.zeros(n + 1, dtype=bool)
    k[sl] = True
    k[0] = False
...
def skeletonize_with_skimage(p):
    if skeletonize_3d is None:
        raise RuntimeError("skeletonize_3d unavailable")
...
def prune_short_end_branches(sk01, lmin):
    ...
    for _ in range(50):
        ...
        ep = sk & (deg == 1)
        jn = sk & (deg >= 3)
        if not ep.any() or not jn.any():
            break

        d = np.full(sk.shape, np.inf, dtype=np.float32)
        d[jn] = 0.0
        fr = jn.copy()
        dd = 0
...
def dilate_erode_bridge(vol01, dilate_iters=0, close_iters=0):
    ...
    if int(dilate_iters) > 0:
        x = ndi.binary_dilation(x, structure=st, iterations=int(dilate_iters))
    if int(close_iters) > 0:
        x = ndi.binary_closing(x, structure=st, iterations=int(close_iters))
    if int(dilate_iters) > 0:
        x = ndi.binary_erosion(x, structure=st, iterations=int(dilate_iters))
    return x.astype(np.uint8)


def make_directional_field(shape, rng, sig_long, sig_short):
    f_z = ndi.gaussian_filter(
        rng.normal(0, 1, size=shape).astype(np.float32),
        sigma=(sig_long, sig_short, sig_short),
    )
    f_y = ndi.gaussian_filter(
        rng.normal(0, 1, size=shape).astype(np.float32),
        sigma=(sig_short, sig_long, sig_short),
    )
    f_x = ndi.gaussian_filter(
        rng.normal(0, 1, size=shape).astype(np.float32),
...
def make_multiscale_anisotropic_field(
    shape,
    rng,
    base_sigma,
    aniso_ratio,
    ...
    bs = float(base_sigma)
    ar = float(aniso_ratio)

    def one(scale, weight):
        sig_long = max(0.8, bs * scale * ar)
        sig_short = max(0.8, bs * scale)
        return weight * make_directional_field(shape, rng, sig_long, sig_short)

    f = (
        one(1.8, coarse_weight)
        + one(1.0, medium_weight)
        + one(0.6, fine_weight)
    )
    return normalize(f)
...
def component_stats_3d(vol01):
    ...
    if n == 0:
        return {"n_components": 0, "lcc_frac": 0.0}
...
def make_proto_and_skeleton(shape, rp, rng, skel_mode, fiji_exe, fiji_cmd, dbg):
    ...
    p01 = dilate_erode_bridge(
        p01,
        dilate_iters=int(rp.bridge_dilate_iters),
    ...
    if bool(rp.use_skeleton):
        if skel_mode == "skimage":
            sr = skeletonize_with_skimage(p01)
            used_skeleton = True
...
def radius_samples_for_skeleton(sk01, rng, br, mode, jit, ss):
    sk = sk01.astype(bool)
    rad = np.zeros(sk01.shape, dtype=np.float32)
    if not sk.any():
        return rad

    b = float(max(0.5, br))
    j = float(np.clip(jit, 0.0, 0.9))
    st = ndi.generate_binary_structure(3, 2)
...
def thicken_from_skeleton_radius_field(sk01, rng, tbvtv, br, rm, rj, rss, rsh, dbg):
    sk = sk01.astype(bool)
    if not sk.any():
        return np.zeros_like(sk01, dtype=np.uint8), {"error": "Empty"}
    ...
    iz, iy, ix = inds
    rf = rs[iz, iy, ix].astype(np.float32)
    mr = float(max(0.5, 0.3 * br))
    rf = np.maximum(rf, mr)

    tgt = float(np.clip(tbvtv, 0.01, 0.95))
    lo, hi = 0.25, 3.0
    bs = float(np.clip(rsh, lo, hi))
    be = float("inf")
...
def microct_gray_solid(bone01, gp, rng, br=2.0):
    ...
    if float(gp.pve_sigma) > 0:
        g = ndi.gaussian_filter(g, sigma=float(gp.pve_sigma))
    if float(gp.bg_tex_sd) > 0:
        g += rng.normal(0, float(gp.bg_tex_sd), size=g.shape).astype(np.float32)
    if float(gp.noise_sd) > 0:
        g += rng.normal(0, float(gp.noise_sd), size=g.shape).astype(np.float32)
...
def measure_all_morphometrics(v, vu):
    ...
    st = ndi.generate_binary_structure(3, 2)
    l, n = ndi.label(bone, structure=st)
    lf = 0.0
    nc = int(n)
    if n > 0:
        c = np.bincount(l.ravel())
...
def skeleton_graph_stats(sk01):
    sk = sk01.astype(bool)
    if not sk.any():
        return {"skel_voxels": 0, "junctions": 0, "endpoints": 0, "ej_ratio": None}
...
def generate_one(params, args, outdir, label="", seed_override=None):
    ...
    bvtv = params["bvtv"]
    tbth = params["tbth_um"]
    tbn = params["tbn_per_mm"]
    tbsp = params["tbsp_um"]
    vu = params.get("voxel_um", float(args.voxel_um or 39.0))
    shape = (
        params.get("shape_z", args.z or 160),
    ...
    rp = RidgeParams(
        ...
        medium_weight=float(args.medium_weight),
        fine_weight=float(args.fine_weight),
        sheet_q=float(args.sheet_q),
    ...
    gp = GrayParams(
        ...
        bg_tex_sd=float(args.bg_tex_sd),
    )
    ...
    met = {
        ...
        "validation": val,
        "tamimi_warnings": tw,
        "skeleton_stats": skeleton_graph_stats(sk01),
        "component_stats": component_stats_3d(bone01),
        "thick_info": ti,
        "proto_info": si,
        "params": {
            "ridge": asdict(rp),
...
def main():
    ...
    if args.voi_dirs is not None:
        ...
        for s in samples:
            print(f"    [{s['sample_index']:03d}] BV/TV={s['bvtv']:.3f} Tb.Th={s['tbth_um']:.0f}um Tb.N={s['tbn_per_mm']:.2f}/mm")
        ...
        print(f"Tamimi {args.profile}")
        params = {
            "bvtv": ref["BVTV"],
            "tbth_um": ref["TbTh_um"],
            "tbn_per_mm": ref["TbN_per_mm"],
            "tbsp_um": ref["TbSp_um"],
