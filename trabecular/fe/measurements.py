"""
trabecular.fe.measurements
==========================
BoneJ-equivalent morphometric measurements implemented in pure Python/NumPy.

Reference: Doube et al. (2010) BoneJ. Bone 47(6):1076–9.

Functions
---------
measure_thickness(mask, voxel_um)          Local thickness via EDT
measure_connectivity(mask)                  Euler number and LCC fraction
measure_anisotropy(mask, voxel_um)         Degree of anisotropy via MIL
measure_smi(mask, voxel_um)               Structure Model Index
measure_all_bonej(mask, voxel_um)         Run all measurements
"""
import numpy as np
import time
...
def measure_thickness(mask, voxel_um, is_bone=True):
    ...
    _empty = {k: 0.0 for k in ['p10','p25','p50','p75','p90','mean','std','max','map']}
    if target.sum() == 0:
        return _empty
...
def measure_anisotropy(mask, voxel_um, n_directions=256, n_lines=80):
    ...
    for d_idx, direction in enumerate(directions):
        ...
        for _ in range(n_lines):
            start    = np.array([rng.integers(0, nz), rng.integers(0, ny),
                                  rng.integers(0, nx)], dtype=float)
            prev_val = 0; intercepts = 0; length = 0; pos = start.copy()
...
def measure_smi(mask, voxel_um):
    ...
    dS  = S_d - S
    SMI = 6 * V * dS / (voxel_um * S ** 2) if S > 0 else float('nan')
...
def measure_all_bonej(mask, voxel_um, include_anisotropy=False,
    ...
    include_anisotropy: run MIL anisotropy (slow, ~30s for 128³)
    include_smi       : run Structure Model Index (moderate speed)
    verbose           : print timing summary

    Returns
    -------
    dict with keys: BVTV, TbTh_um_*, TbSp_um_*, TbN_per_mm, Euler,
                    connectivity, n_components, lcc_frac, thickness_map,
                    spacing_map [, DA, SMI]
    """
    t0   = time.time()
    ...
    if verbose:
        print(f"BoneJ measurements: {time.time()-t0:.1f}s | "
              f"BV/TV={bvtv:.3f}, Tb.Th={tbth['p50']:.0f} µm, "
              f"Tb.N={tbn:.2f} /mm")
