...
v15: Pooled VOI-driven trabecular generator for ML datasets.
v14 core (proven geometry) + grayscale matching + tightened sampler + VOI presets.
...
VOI_PRESETS = {"voi1":{"xy":300,"z":432},"voi2":{"xy":152,"z":432},"voi3":{"xy":152,"z":432},"voi4":{"xy":152,"z":432},"voi5":{"xy":152,"z":432}}
TAMIMI_BOUNDS = {"BVTV":(0.05,0.50),"TbTh_um":(80.0,300.0),"TbN_per_mm":(0.5,5.0),"TbSp_um":(150.0,1200.0)}
...
def load_all_voi_targets(voi_dirs, voxel_um):
    ...
    for d in voi_dirs:
        for f in sorted(Path(d).glob("*_targets.json")):
            ...
            data["_src"] = str(f); all_t.append(data); print(f"  Loaded: {f.name}")
    ...
    bv,th,sp = np.array(bv),np.array(th),np.array(sp)
    tn = bv/(th/1000.0+1e-9)
    def st(a): return {"mean":float(a.mean()),"std":float(a.std()),"min":float(a.min()),"max":float(a.max())}
    p = {"n":len(all_t),"voxel_um":voxel_um,"BVTV":st(bv),"TbTh_um":st(th),"TbSp_um":st(sp),"TbN_per_mm":st(tn)}
    print(f"  BV/TV: {p['BVTV']['mean']:.3f}+/-{p['BVTV']['std']:.3f} [{p['BVTV']['min']:.3f}-{p['BVTV']['max']:.3f}]")
...
def extract_single_params(voi, args):
    vu=args.voxel_um or 39.0; rv=1.0; vz=voi.get("voxel_um_zyx")
    if vz and len(vz)>=1: rv=float(vz[0])
    c=vu/max(1.0,rv)
    ...
    sh=voi.get("shape_zyx"); sz=args.z or (int(sh[0]) if sh else 160); sxy=args.xy or (int(sh[1]) if sh else 300)
    return {"bvtv":float(bv),"tbth_um":float(tbth),"tbn_per_mm":float(tbn),"tbsp_um":float(tbsp),"voxel_um":float(vu),"shape_z":sz,"shape_xy":sxy}

@dataclass
class RidgeParams:
    base_sigma:float=3.8; warp_sigma:float=14.0; warp_amp:float=4.8; hessian_sigma:float=1.4; ridge_strength:float=1.0
...
class GrayParams:
    ...
    pve_sigma:float=0.5; noise_sd:float=4.0; bg_tex_sd:float=2.0; unsharp:float=0.6; unsharp_sigma:float=0.8

def save_png_u8(img,path): path.parent.mkdir(parents=True,exist_ok=True); Image.fromarray(img.astype(np.uint8),mode="L").save(path)
def save_tif_u8(s,path): path.parent.mkdir(parents=True,exist_ok=True); tiff.imwrite(path,s.astype(np.uint8),imagej=True,dtype=np.uint8)
def save_json(o,path): path.parent.mkdir(parents=True,exist_ok=True); json.dump(o,open(path,"w"),indent=2)
...
def normalize(f): x=f.astype(np.float32); x-=float(x.mean()); x/=float(x.std()+1e-6); return x
def smooth_warp(field,rng,ws,wa):
    if wa<=0: return field
    sh=field.shape; dz=ndi.gaussian_filter(rng.normal(0,1,sh),sigma=ws)*wa; dy=ndi.gaussian_filter(rng.normal(0,1,sh),sigma=ws)*wa; dx=ndi.gaussian_filter(rng.normal(0,1,sh),sigma=ws)*wa
    Z,Y,X=sh; zz,yy,xx=np.meshgrid(np.arange(Z),np.arange(Y),np.arange(X),indexing="ij")
    return map_coordinates(field,np.array([zz+dz,yy+dy,xx+dx]),order=1,mode="reflect").astype(np.float32)
def hessian_eigs_3d(f,sigma):
    fxx=ndi.gaussian_filter(f,sigma=sigma,order=(0,0,2)); fyy=ndi.gaussian_filter(f,sigma=sigma,order=(0,2,0)); fzz=ndi.gaussian_filter(f,sigma=sigma,order=(2,0,0))
    fxy=ndi.gaussian_filter(f,sigma=sigma,order=(0,1,1)); fxz=ndi.gaussian_filter(f,sigma=sigma,order=(1,0,1)); fyz=ndi.gaussian_filter(f,sigma=sigma,order=(1,1,0))
    H=np.stack([np.stack([fzz,fyz,fxz],axis=-1),np.stack([fyz,fyy,fxy],axis=-1),np.stack([fxz,fxy,fxx],axis=-1)],axis=-2)
    w=np.linalg.eigvalsh(H.reshape(-1,3,3)).reshape(f.shape+(3,)); idx=np.argsort(np.abs(w),axis=-1); w=np.take_along_axis(w,idx,axis=-1)
    return w[...,0],w[...,1],w[...,2]
def vesselness_ridge(f,sigma):
    l1,l2,l3=hessian_eigs_3d(f,sigma=sigma); eps=1e-6; r1=np.abs(l1)/(np.abs(l3)+eps); r2=np.abs(l2)/(np.abs(l3)+eps)
    V=np.exp(-(r1*r1)/0.25)*np.exp(-(r2*r2)/0.25); return (V/(float(V.max())+1e-6)).astype(np.float32)
...
def keep_largest_component(v):
    st=ndi.generate_binary_structure(3,2); l,n=ndi.label(v.astype(bool),structure=st)
    if n==0: return v.astype(np.uint8)
    c=np.bincount(l.ravel()); c[0]=0; return (l==int(c.argmax())).astype(np.uint8)
def remove_small_components(v,ms):
    if int(ms)<=0: return v.astype(np.uint8)
    st=ndi.generate_binary_structure(3,2); l,n=ndi.label(v.astype(bool),structure=st)
    if n==0: return v.astype(np.uint8)
    c=np.bincount(l.ravel()); k=c>=int(ms); k[0]=False; return k[l].astype(np.uint8)
...
def hysteresis_on_response(R,ql,qh):
    ...
    s=R>=th; w=R>=tl; st=ndi.generate_binary_structure(3,2); l,n=ndi.label(w,structure=st)
...
def neighbor_degree_26(sk):
    st=ndi.generate_binary_structure(3,2); n=ndi.convolve(sk.astype(np.uint8),st.astype(np.uint8),mode="constant",cval=0)
    return (n-sk.astype(np.uint8)).astype(np.int16)
def prune_short_end_branches(sk01,lmin):
    lmin=int(max(1,lmin)); st=ndi.generate_binary_structure(3,2); sk=sk01.astype(bool); rm=0
    for _ in range(50):
        deg=neighbor_degree_26(sk.astype(np.uint8)); ep=sk&(deg==1); jn=sk&(deg>=3)
        if not ep.any() or not jn.any(): break
        d=np.full(sk.shape,np.inf,dtype=np.float32); d[jn]=0.0; fr=jn.copy(); dd=0
        while dd<lmin and fr.any():
            dd+=1; nb=ndi.binary_dilation(fr,structure=st)&sk&(d==np.inf); d[nb]=float(dd); fr=nb
...
def make_proto_and_skeleton(shape,rp,rng,skel_mode,fiji_exe,fiji_cmd,dbg):
    ...
    p01=morph_iters(p01,"close",int(rp.proto_close_iters)); p01=morph_iters(p01,"open",int(rp.proto_open_iters))
    p01=remove_small_components(p01,int(rp.proto_min_component))
    st26=ndi.generate_binary_structure(3,2)
    if p01.astype(bool).sum()>0: p01=ndi.binary_closing(p01.astype(bool),structure=st26,iterations=1).astype(np.uint8)
    sr=p01.copy().astype(np.uint8); us=False
    if bool(rp.use_skeleton):
        if skel_mode=="skimage": sr=skeletonize_with_skimage(p01); us=True
    sp,pi=prune_short_end_branches(sr,lmin=int(rp.skeleton_prune_lmin))
    if int(rp.reconnect_close_iters)>0: sp=morph_iters(sp,"close",int(rp.reconnect_close_iters))
    if dbg:
        save_tif_u8((R*255).astype(np.uint8),dbg/"ridge_response.tif"); save_tif_u8((p01*255).astype(np.uint8),dbg/"proto_network.tif")
        save_tif_u8((sr*255).astype(np.uint8),dbg/"skeleton_raw.tif"); save_tif_u8((sp*255).astype(np.uint8),dbg/"skeleton_pruned.tif")
    return sp.astype(np.uint8),{"hysteresis":hy,"used_skeleton":us,"skel_mode":skel_mode,"prune":pi}
...
def radius_samples_for_skeleton(sk01,rng,br,mode,jit,ss):
    ...
    b=float(max(0.5,br)); j=float(np.clip(jit,0.0,0.9)); st=ndi.generate_binary_structure(3,2)
    if mode=="branch":
        l,n=ndi.label(sk,structure=st)
        for i in range(1,n+1): rad[l==i]=b*float(np.exp(rng.normal(0.0,0.35*j)))
    else:
        ns=rng.normal(0.0,1.0,size=sk01.shape).astype(np.float32); rad[sk]=b*np.clip(1.0+j*ns[sk],0.25,3.0)
    if float(ss)>0:
        w=sk.astype(np.float32); num=ndi.gaussian_filter(rad,sigma=float(ss)); den=ndi.gaussian_filter(w,sigma=float(ss))+1e-6
...
def thicken_from_skeleton_radius_field(sk01,rng,tbvtv,br,rm,rj,rss,rsh,dbg):
    ...
    dist,inds=ndi.distance_transform_edt(~sk,return_indices=True); iz,iy,ix=inds; rf=rs[iz,iy,ix].astype(np.float32)
    mr=float(max(0.5,0.3*br)); rf=np.maximum(rf,mr)
    tgt=float(np.clip(tbvtv,0.01,0.95)); lo,hi=0.25,3.0; bs=float(np.clip(rsh,lo,hi)); be=float("inf")
    for _ in range(24):
    ...
    return bone,{"base_r":float(br),"min_r":float(mr),"scale":float(bs),"bvtv_tgt":float(tgt),"bvtv_got":float(bone.mean())}

def microct_gray_solid(bone01,gp,rng,br=2.0):
    bone=bone01.astype(bool); d=ndi.distance_transform_edt(bone).astype(np.float32)
    ...
    if float(gp.unsharp)>0: bl=ndi.gaussian_filter(g,sigma=max(0.4,float(gp.unsharp_sigma))); g+=float(gp.unsharp)*(g-bl)
    return np.clip(g,0,255).astype(np.uint8)

def measure_all_morphometrics(v,vu):
    bone=v.astype(bool); bv=float(bone.mean())
    db=ndi.distance_transform_edt(bone)*float(vu); dm=ndi.distance_transform_edt(~bone)*float(vu)
    ...
    tm=(2*float(np.mean(tv))/1000.0) if tv.size else 1e-6; tn=bv/tm if tm>0 else 0.0
    eu=float(euler_number(bone,connectivity=3)); st=ndi.generate_binary_structure(3,2); l,n=ndi.label(bone,structure=st)
    lf=0.0; nc=int(n)
    if n>0: c=np.bincount(l.ravel()); c[0]=0; lf=float(c.max())/float(max(1,bone.sum()))
    return {"BVTV":bv,"TbTh_um_p50":t50,"TbTh_um_p90":t90,"TbSp_um_p50":s50,"TbSp_um_p90":s90,"TbN_per_mm":float(tn),"Euler":eu,"ConnProxy":float(1-eu),"n_components":nc,"lcc_frac":lf}

def skeleton_graph_stats(sk01):
    sk=sk01.astype(bool)
    if not sk.any(): return {"skel_voxels":0,"junctions":0,"endpoints":0,"ej_ratio":None}
    deg=neighbor_degree_26(sk.astype(np.uint8)); ep=int((sk&(deg==1)).sum()); jn=int((sk&(deg>=3)).sum())
    return {"skel_voxels":int(sk.sum()),"junctions":jn,"endpoints":ep,"ej_ratio":float(ep)/max(1,jn) if jn>0 else None}

def validate_morphometrics(m,t):
    ch={}
    for mk,tk,tol,lb in [("BVTV","bvtv_target",0.05,"BV/TV"),("TbTh_um_p50","tbth_um_target",0.15,"Tb.Th"),("TbN_per_mm","tbn_target",0.20,"Tb.N"),("TbSp_um_p50","tbsp_um_target",0.15,"Tb.Sp")]:
        tv=t.get(tk); mv=m.get(mk)
        if tv and mv and float(tv)>0: re=abs(float(mv)-float(tv))/float(tv); ch[lb]={"measured":float(mv),"target":float(tv),"rel_error":re,"pass":bool(re<=tol)}
    lcc=m.get("lcc_frac",0.0); ch["LCC"]={"lcc_frac":float(lcc),"pass":bool(lcc>=0.80)}
    return ch
...
def check_tamimi_bounds(m):
    ...
    for mk,bk in [("BVTV","BVTV"),("TbTh_um_p50","TbTh_um"),("TbN_per_mm","TbN_per_mm"),("TbSp_um_p50","TbSp_um")]:
        ...
        if v is not None and b is not None:
            if float(v)<b[0] or float(v)>b[1]: w.append(f"{mk}={float(v):.2f} outside [{b[0]},{b[1]}]")
    return w
...
def generate_one(params,args,outdir,label="",seed_override=None):
    ...
    bvtv=params["bvtv"]; tbth=params["tbth_um"]; tbn=params["tbn_per_mm"]; tbsp=params["tbsp_um"]
    ...
    if int(args.min_component_size)>0: bone01=remove_small_components(bone01,int(args.min_component_size))
    if bool(int(args.enforce_lcc)): bone01=keep_largest_component(bone01)
    void01=(1-bone01).astype(np.uint8); Z=shape[0]
    save_tif_u8((bone01*255).astype(np.uint8),outdir/"mask.tif"); save_tif_u8((void01*255).astype(np.uint8),outdir/"void.tif")
    save_png_u8((bone01[Z//2]*255).astype(np.uint8),outdir/"mid.png")
    if gp.write_gray:
        gray=microct_gray_solid(bone01,gp,rng,br=br); save_tif_u8(gray,outdir/"gray.tif"); save_png_u8(gray[Z//2],outdir/"gray_mid.png")
    morph=measure_all_morphometrics(bone01,vu)
    tgt={"bvtv_target":bvtv,"tbth_um_target":tbth,"tbn_target":tbn,"tbsp_um_target":tbsp}
    val=validate_morphometrics(morph,tgt); tw=check_tamimi_bounds(morph)
    met={"version":"v15","label":label,"seed":seed,"morphometrics":morph,"targets":tgt,"validation":val,"tamimi_warnings":tw,"skeleton_stats":skeleton_graph_stats(sk01),"thick_info":ti,"params":{"ridge":asdict(rp),"gray":asdict(gp)},"shape_zyx":list(shape),"voxel_um":vu}
...
def build_parser():
    p=argparse.ArgumentParser(description="v15 trabecular generator (v14 core, VOI-pooled, micro-CT matched)")
    p.add_argument("--voi-dirs",nargs="+",type=str,default=None); p.add_argument("--targets-json",type=str,default=None); p.add_argument("--profile",type=str,default=None,choices=["tamimi-hf","tamimi-hoa"])
    p.add_argument("--outdir",type=str,default="output/ml_dataset"); p.add_argument("--num-samples",type=int,default=10); p.add_argument("--base-seed",type=int,default=42)
    ...
    p.add_argument("--proto-q-hi",type=float,default=0.92); p.add_argument("--proto-q-lo",type=float,default=0.84)
    p.add_argument("--proto-close-iters",type=int,default=2); p.add_argument("--proto-open-iters",type=int,default=0); p.add_argument("--proto-min-component",type=int,default=400)
    p.add_argument("--use-skeleton",type=int,default=1); p.add_argument("--skeleton-mode",type=str,default="skimage",choices=["skimage","fiji"])
    p.add_argument("--fiji-exe",type=str,default=None); p.add_argument("--fiji-command",type=str,default="Skeletonize (2D/3D)")
...
def main():
    ...
    if args.voi_preset:
        ...
        print(f"VOI preset '{args.voi_preset}': {pr['xy']}x{pr['xy']}x{pr['z']}")
    if args.voi_dirs is not None:
        ...
        prng=np.random.default_rng(int(args.base_seed)); samples=sample_targets_from_pool(pooled,prng,int(args.num_samples))
        print(f"\n  Sampled {len(samples)} targets (Tamimi-clamped):")
        for s in samples: print(f"    [{s['sample_index']:03d}] BV/TV={s['bvtv']:.3f} Tb.Th={s['tbth_um']:.0f}um Tb.N={s['tbn_per_mm']:.2f}/mm")
        am=[]
        for s in samples:
            s["voxel_um"]=float(args.voxel_um); s["shape_z"]=args.z or 160; s["shape_xy"]=args.xy or 300
            am.append(generate_one(s,args,Path(args.outdir)/f"sample_{s['sample_index']:03d}",label=f"sample_{s['sample_index']:03d}",seed_override=int(args.base_seed)+s["sample_index"]+1))
        save_json({"version":"v15","n":len(am),"pooled":pooled,"gray":{"marrow":args.marrow_mean,"bone":args.bone_mean},"samples":[{"label":m["label"],"seed":m["seed"],"bvtv":m["targets"]["bvtv_target"],"bvtv_m":m["morphometrics"]["BVTV"]} for m in am]},Path(args.outdir)/"dataset_manifest.json")
        print(f"\n{'='*60}\n  Dataset: {len(am)} samples in {args.outdir}/\n{'='*60}")
    elif args.targets_json is not None:
        voi=load_voi_targets(args.targets_json); params=extract_single_params(voi,args); generate_one(params,args,Path(args.outdir),label="single")
    elif args.profile is not None:
        ref=TAMIMI_HF if args.profile=="tamimi-hf" else TAMIMI_HOA; print(f"Tamimi {args.profile}")
        params={"bvtv":ref["BVTV"],"tbth_um":ref["TbTh_um"],"tbn_per_mm":ref["TbN_per_mm"],"tbsp_um":ref["TbSp_um"],"voxel_um":float(args.voxel_um),"shape_z":args.z or 160,"shape_xy":args.xy or 300}
        generate_one(params,args,Path(args.outdir),label="literature")
    else: print("Provide: --voi-dirs, --targets-json, or --profile"); raise SystemExit(1)
